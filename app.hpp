...
class VkInit {
    ...
    void pickPhysicalDevice() {
        ...
        if(deviceCount==0) err("failed to find vulkan compatible GPUs");
        ...
        if (physicalDevice == VK_NULL_HANDLE)
            err("failed to find a vulkan compatible GPU");
    ...
    GLFWwindow* window;
    VkInit(std::unique_ptr<VkApp> vkApp): app{std::move(vkApp)} {}
    void run() {
        initWindow();
        initVulkan();
        app->init();
